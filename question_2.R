...
library(dplyr)
library(sf)
style_file("/Users/yeong/Documents/GitHub/problem-set-2-chaeyeonguc/DAP_2.R")
...
choropleth_public_schools <- ggplot() +
  ...
  labs(title = "Location of Public Schools by Safety Scores in Chicago",       
       fill = "Safety Score",
...
speed_cameras_sf <- speed_cameras_sf %>%
  mutate(
    GO.LIVE.DATE = as.Date(GO.LIVE.DATE, format = "%m/%d/%Y"),  
    active_days = as.numeric(Sys.Date() - GO.LIVE.DATE)  
  )

### Creating a choropleth combined with the shapefile
choropleth_speed_cameras <- ggplot() +
  geom_sf(data = zip_codes) +
  geom_sf(data = speed_cameras_sf, aes(fill = active_days), shape = 21, size = 3) +
  scale_fill_gradient(low = "white", high = "red") +
  labs(
    title = "Location of Speed Cameras by the Number of Active Days in Chicago",
    fill = "The Number of Active Days",
